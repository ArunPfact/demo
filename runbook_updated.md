...
- vsta_scrape_run Currently stoped beacuse of cloud fare security issues.
...
- **check if user credentials are hardocded in the scripts.**
  ...
  - API key is hardcoded in the `save_ads_new_method`
  - proxies are hardcoded in the 'save_ads_new_method'
...
- **Steps to execute (manually):** Connect to the host server provided above and navigate to the script path or just directly type `python3 /root/Desktop/shopify_products/shopify_products_optimized_run.py`
...
- **Additional Notes:**
  ...
  - Calls the `extract_products` function from the `shopify_products_optimized` module to gather product details from the respective Shopify website.
...
- **Steps to execute (manually):** Connect to the host server provided above and navigate to the script path or just directly type `python3 /root/Desktop/scrapping_full/cron/separate_cron/trustpilot_review_updated_run.py`
...
- **Steps to execute (manually):** Connect to the host server provided above and navigate to the script path or just directly type `python3 /root/Desktop/scrapping_full/cron/separate_cron/shopify_store_reviews_updated_run.py`
...
- **Path to script:** `python3 /root/Desktop/Similarweb/similarweb_new_run.py`
- **Steps to execute (manually):** Connect to the host server provided above and navigate to the script path or just directly type `python3 /root/Desktop/Similarweb/similarweb_new_run.py`
- **Logs:** Logs are stoed at logs/similarweb*update*{date}.log
- **Related Custom Python Packages/Files:** similarweb_new_updated
- **Additional Notes:**
  - Runs on multiple servers.
  - This script gets the brands from `shopify_master_only` function.
  - Resets the index of `similarweb_brands` data frame.
  - Calls 'similarwebscrapping' function from the 'similarweb_new_updated' module for the entire list of brands.
...
- **Path to script:** `python3 /root/Desktop/scrapping_full/cron/separate_cron/shopify_theme_updated_run.py`
- **Steps to execute (manually):** Connect to the host server provided above and navigate to the script path or just directly type `python3 /root/Desktop/scrapping_full/cron/separate_cron/shopify_theme_updated_run.py`
